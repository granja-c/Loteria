...
def clicked(button):
  ...
  if button == button4:
    ...
    clickedbs.append("b4")
    if clickedbs.count("b1") > 0 and clickedbs.count("b2") > 0 and clickedbs.count("b3") > 0 and clickedbs.count("b4") > 0 and clickedbs.count("b5") > 0:
      tk.messagebox.askyesno(title = "Bingo!", message = "You win!")
...
def next():
  testlbl.configure(text = random.choice(letterlist) + random.choice(numberlist))
...
testlbl = tk.Label(text= random.choice(letterlist) + numberlist[0], bg = pink, height = 6, width = 9)
