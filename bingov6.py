...
class Game(tk.Toplevel):
  def __init__(self, parent):
    ...
    options.remove(n)
    testlbl = tk.Label(self, bg = pink, text = n, height = 6, width = 9)
    ...
    def next():
      ...
      if button31['text'] == testlbl['text']:
        button31.configure(bg = pink)
        clickedbs.append("b31")
      if button32['text'] == testlbl['text']:
        button32.configure(bg = pink)
        clickedbs.append("b32")
      if button33['text'] == testlbl['text']:
        button33.configure(bg = pink)
        clickedbs.append("b33")
      if button34['text'] == testlbl['text']:
        button34.configure(bg = pink)
        clickedbs.append("b34")
      if button35['text'] == testlbl['text']:
        button35.configure(bg = pink)
        clickedbs.append("b35")
      ...
      if clickedbs.count("b31") > 0 and clickedbs.count("b32") > 0 and clickedbs.count("b33") > 0 and clickedbs.count("b34") > 0 and clickedbs.count("b35") > 0:
        tk.messagebox.showinfo(parent = self, title = "Bingo!", message = "You lose!")
